...
var logipar_Logipar = $hx_exports["logipar"]["Logipar"] = function(custom_operators) {
	...
	if(custom_operators != null) {
		console.log(custom_operators);
		var key = custom_operators.keys();
		while(key.hasNext()) {
			var key1 = key.next();
			var _this = this.syntax;
			if(__map_reserved[key1] != null ? _this.existsReserved(key1) : _this.h.hasOwnProperty(key1)) {
				var this1 = this.syntax;
				var value = __map_reserved[key1] != null ? custom_operators.getReserved(key1) : custom_operators.h[key1];
				var _this1 = this1;
...
logipar_Logipar.prototype = {
	...
	,tokenType: function(token) {
		...
		return new logipar_Token("LITERAL",token);
	}
...
var logipar_Node = $hx_exports["logipar"]["Node"] = function(token) {
	this.token = token;
};
...
logipar_Node.prototype = {
	...
	,fancyString: function(f) {
		...
		case "NOT":
			return "NOT(" + this.right.fancyString(f) + ")";
		default:
			return "(" + this.left.fancyString(f) + " " + Std.string(this.token.type) + " " + this.right.fancyString(f) + ")";
		}
	}
	,check: function(a,f) {
		switch(this.token.type) {
...
var logipar_Token = $hx_exports["logipar"]["Token"] = function(type,literal) {
	this.type = type;
